...
public class SectionRenderDataUnsafe {
    ...
    /**
     * Combined bounds check and offset calculation for facing-indexed data.
     * Merging these operations helps HotSpot optimize repeated accesses to the same facing.
     */
    private static long facingOffset(int facing) {
        // Bitwise check: (facing & ~7) != 0 catches values outside 0-7 in one operation
        // Combined with facing >= NUM_FACINGS to catch 7 (since NUM_FACINGS is 7)
        if ((facing & ~7) != 0 || facing >= NUM_FACINGS) {
            throw new IndexOutOfBoundsException("facing must be 0-" + (NUM_FACINGS - 1) + ", got: " + facing);
        }
        return OFFSET_SLICE_RANGES + (facing * DATA_PER_FACING_SIZE);
    }
    ...
    public static void setElementCount(long ptr, int facing, int value) {
        MemoryUtil.memPutInt(ptr + facingOffset(facing) + 4L, value);
    }
