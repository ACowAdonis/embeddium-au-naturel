...
public class BiomeColorCache {
    ...
    public int getColor(ColorResolver resolver, int blockX, int blockY, int blockZ) {
        ...
        // Use single lookup instead of containsKey + get (double-lookup pattern)
        Slice[] sliceArray = this.slices.get(resolver);
        if (sliceArray == null) {
            sliceArray = this.createSlices(resolver);
        }
    ...
    private Slice[] createSlices(ColorResolver resolver) {
        var sliceArray = new Slice[this.sizeY];

        for (int y = 0; y < this.sizeY; y++) {
            sliceArray[y] = new Slice(this.sizeXZ);
        }
