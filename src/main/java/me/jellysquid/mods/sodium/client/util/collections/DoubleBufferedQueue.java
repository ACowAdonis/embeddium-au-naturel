...
public final class DoubleBufferedQueue<E> {
    ...
    private static final class QueueImpl<E> implements ReadQueue<E>, WriteQueue<E> {
        ...
        QueueImpl() {
            // Start with a larger initial capacity to avoid repeated resizing during occlusion culling.
            // For modern render distances (16+ chunks), the culler visits thousands of sections per frame.
            // Starting at 256 causes repeated growth (256→512→1024→2048→4096), each with an array copy.
            // 4096 eliminates most resize operations for typical gameplay with negligible memory cost (~32KB).
            this(4096);
        }
